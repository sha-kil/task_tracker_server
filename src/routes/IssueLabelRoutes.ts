...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const projectBoard = await prisma.project.findFirst({
      where: {
    ...
    if (projectBoard === null) {
      console.error("Project board not found:", createData.data.projectId)
    ...
    res
      .status(500)
      .json({ error: "Failed to create issue label" })
  }
})

async function getIssueLabelsByProjectId(projectId: string, res: any) {
  try {
...
async function getIssueLabelsByIssueId(issueId: string, res: any) {
  try {
    const issueLabels = await prisma.issueLabel.findMany({
      where: {
        issues: {
